...
% ofiles = 'OTFbead640.tif';ifiles = 'PSF640.tif';linespacing = 0.5;lambda = 640;ileavekz = [0, 0, 10,11, 0, 0, 0];
% ofiles = 'OTFbead561.tif';ifiles = 'PSF561.tif';linespacing = 0.45;lambda = 561;ileavekz = [0, 0, 9,9, 0, 0, 0];
ofiles = 'OTFbead473.tif';ifiles = 'PSF473.tif';linespacing = 0.4;lambda = 473;ileavekz = [0, 0, 9,9, 0, 0, 0];
% NA = 1.5;NIMM = 1.518;      % oil
% NA = 1.3;NIMM = 1.406;    % silicon oil
NA = 1.2;NIMM = 1.33;     % water
nphases = 3;
%%
...
do_compen = 0;                          % Whether to compensate finite bead size
I2M_inc = 0;
...
if (Generate_bands)
    ...
    minval = min(abs(floatimage(:)));
    maxval = max(abs(floatimage(:)));
...
%     Stack_bands = permute(floatimage(:, :, :, [1 2 4]), [2 1 4 3]);
    Stack_bands = permute(floatimage(:, :, :, [1 2]), [2 1 4 3]);
    Stack_bands = reshape(Stack_bands, [1, numel(Stack_bands)]);
...
for i = 0 : (norders - 1)
    if i == 0
        ...
        Stack_band0_exp(Stack_band0_exp >= 1) = 1;
        Stack_band0_exp = Stack_band0_exp ./ 1;
...
%         Stack_band0 = permute(sum(abs(avg_output(:, :, [1 2 * i])),3), [2 1]);
...
background = sum(sum(image .* image_mask)) / sum(sum(image_mask));
...
% Caluclate background
infocus_sec = floor(zc);
t = stack5phases(1:floor(yc) - 20, :, infocus_sec, :);
background = sum(t(:)) / (nphases * (floor(yc) - 20) * nx);
