classdef SIM_Reconstructor
    properties
        ...
        m_myParams = struct(...
            ...
            'bsavewf', [],...              % Whether to save fft data
    ...
    methods (Access = public)
        function obj = SIM_Reconstructor(arg)
            ...
            obj.m_imgParams.ntimes = size(obj.m_all_matching_files, 1);
        ...
        function obj = processOneVolume(obj, it)
            ...
            if(obj.m_myParams.GPUcomputing)
                obj.m_reconData.savedBands = gpuArray(obj.m_reconData.savedBands);
                obj.m_reconData.overlap0 = gpuArray(obj.m_reconData.overlap0);
                obj.m_reconData.overlap1 = gpuArray(obj.m_reconData.overlap1);
                obj.otf.vals = gpuArray(obj.otf.vals);
            end
...
%            obj.m_reconData.savedBands = gather(obj.m_reconData.savedBands);
            % RL part
            if (obj.m_myParams.bRLonOutput)
                ...
                ImageJ_formatted_TIFF.WriteTifStack(gather(log(fftshift(abs(obj.m_reconData.RLbuffer)))), 'RLbuffer.tif');
                obj.m_reconData.SIM_OTF = obj.m_reconData.SIM_OTF ./ obj.m_reconData.SIM_OTF(1, 1, 1);
                obj.m_reconData.RLbuffer = gather(deconvolve(obj.m_reconData.RLbuffer, obj.m_reconData.SIM_OTF, obj.m_myParams.iter_output, true));
            end
        ...
        function obj = writeResult(obj, it, iw)
            ...
            if (obj.m_myParams.bRLonOutput)
                ...
                SIM_OTF = gather(log(abs(fftshift(obj.m_reconData.SIM_OTF))));
                ...
                minval = min(obj.m_reconData.RLbuffer(:));
                maxval = max(obj.m_reconData.RLbuffer(:));
                ...
                minval = min(RLbuffer_fft(:));
                maxval = max(RLbuffer_fft(:));
...
%                 t = abs(ifftn(obj.m_reconData.outbuffer));
                % Write Wiener-filtered fft data image
                if (obj.m_myParams.bsavefft)
                    ...
                    fileID = fopen(strcat('DataForTest\Wiener_fft_' , num2str(it), '.tif'), 'w+');
                    [Height, Width, Depth] = size(outbuffer_fft);
                    minval = min(outbuffer_fft(:));
                    maxval = max(outbuffer_fft(:));
                    header = ImageJ_formatted_TIFF.write_IFD(fileID, Width, Height, class(outbuffer_fft), ...
                        1, Depth, 1, obj.m_imgParams.dz / obj.m_myParams.z_zoom, minval, maxval, obj.m_imgParams.dy / obj.m_myParams.zoomfact);
                    outbuffer_fft = permute(outbuffer_fft, [2 1 3]);
                    outbuffer_fft = reshape(outbuffer_fft, [1, Height * Width * Depth]);
                ...
                if obj.m_myParams.singlemax == -1
                    obj.m_myParams.singlemax = max(outbuffer(:));
                end
                outbuffer = im2uint16(outbuffer/obj.m_myParams.singlemax);      % 这一行是自己加的，为了输出为uint16，但是这会导致每一个体积的最大值都是65535
                foldername = strcat(obj.m_myParams.ifiles, '\sr');
                if ~exist(foldername, 'dir')
                    mkdir(foldername);
                end
                fileID = fopen(strcat(foldername, '\Wiener_' , num2str(it), '.tif'), 'w+');
                [Height, Width, Depth] = size(outbuffer);
                minval = min(outbuffer(:));
                maxval = max(outbuffer(:));
...
%                 obj.m_myParams.linespacing = 0.198802;
            ...
            if isfield(arg, 'savewf')
                obj.m_myParams.bsavewf = true;
            else
                obj.m_myParams.bsavewf = false;
            end
            ...
            if isfield(arg, 'GPUcomputing')
                obj.m_myParams.GPUcomputing = true;
            else
                obj.m_myParams.GPUcomputing = false;
            end
        end
...
% 由于字符串排序会使“1X”号排在“1”号和“2”号之间，因此需要转换成数值重新排序
l = strlength(pattern);
maxnum = size(allfiles,1);
seqbuf = zeros(maxnum,2);
cnt = 0;
for i = 1 : maxnum
    if contains(allfiles(i).name, pattern)
        cnt = cnt+1;
        matchingfiles{cnt,1} = strcat(allfiles(i).folder, '\', allfiles(i).name); % Dynamic, cannot be preallocated.
        seqbuf(cnt,1) = str2double(allfiles(i).name(l+1:end-4));
    end
end
seqbuf = seqbuf(1:cnt,:);
seqbuf(:,2) = 1:cnt;
seqbuf = sortrows(seqbuf,1);
matchingfiles = matchingfiles(seqbuf(:,2));
end
...
for direction = 1 : params.ndirs
    ...
    for z = 1 : imgParams.nz
        ...
        for phase = 1 : params.nphases
            ...
            t = rawImages(:, :, z + zoffset, phase);
            sum_buffer(phase) = sum(t(:));
        end
...
t = sum(images, 5);
wide_field = sum(t,4) / params.nphases / params.ndirs;
[Height, Width, Depth] = size(wide_field);
if(params.wfmax == -1)
    params.wfmax = max(wide_field(:));
end
wide_field = im2uint16(wide_field/params.wfmax);      % 这一行是自己加的，为了输出为uint16，但是这会导致每一个体积的最大值都是65535
...
foldername = strcat(params.ifiles, '\wf');
if ~exist(foldername, 'dir')
    mkdir(foldername);
end
fileID = fopen(strcat(foldername, '\wide_field_', num2str(it), '.tif'), 'w+');
[Height, Width, Depth] = size(wide_field);
minval = min(wide_field(:));
maxval = max(wide_field(:));
...
if (params.bsavefft)
    ...
    minval = min(wide_field_fft(:));
    maxval = max(wide_field_fft(:));
...
wide_field = sum(images, 4) / params.nphases / params.ndirs;
...
if (true)
    ...
    minval = min(outputOtf_buffer(:));
    maxval = max(outputOtf_buffer(:));
...
minval = min(wide_field_RL(:));
maxval = max(wide_field_RL(:));
...
% data.savedBands = gather(data.savedBands);
...
minval = min(separated_bands(:));
maxval = max(separated_bands(:));
...
if (imgParams.nz > 1)
    minDist = 0.5 * otf.getCutoff() / dkr;  % 3D-SIM
else
...
beta = asin(k0mag / (2.0 / lambdaexc_medium));          % 非三光束这个值可能会变



% "zdistcutoff" is the axial pixels of maximum frequency of emmision OTF;
if ~(params.bTwolens)
    zdistcutoff = floor(((1.0 - cos(alpha)) / lambdaem) / imgParams.dkz);
else
    if (order2 == 1)
...
overlap_mask((rdist1 <= rdistcutoff) & (rdist12 <= rdistcutoff) & (abs(otf1) > otfcutoff)) = 1.0;
...
phase_shift = single(2.0 * pi * (X .* kx ./ imgParams.nx + Y .* ky ./ imgParams.ny));   % 2pi*k*r
...
t = sum((conj(overlap0) .* overlap1), 3) .* exp(-1j .* phase_shift);
XStarYFR = sum(t(:));
sumXMagFR = sum(abs(overlap0(:)) .^ 2);
sumYMagFR = sum(abs(overlap1(:)) .^ 2);
%{
...
modamp1 = gather(XStarYFR / sumXMagFR); % modamp
modamp2 = gather(XStarYFR / sumYMagFR); % amp_inv
...
modamp3 = gather(tan(beta) * exp(1j * angle(XStarYFR)));
...
if (~params.bTwolens)
    ...
    imgParams.zdistcutoff(1) = 2*ceil(((1 - cos(alpha)) / lambdaem) / imgParams.dkz);
...
if(params.GPUcomputing)
        shifted = gpuArray(shifted);
        otfSim = gpuArray(otfSim);
    end
...
% 频谱信号在四个角，中间的十字用0填充即可，然后利用傅里叶变换亚像素平移
shifted = zeros([params.zoomfact * imgParams.ny, params.zoomfact * imgParams.nx, params.z_zoom * imgParams.nz0, params.nphases], 'single');
if(params.GPUcomputing)
    shifted = gpuArray(shifted);
end
...
fprintf("\n");
% 为了各向同性，如果只有一个方向，对一级光设一个半径为条纹矢量的截止区域
apo2 = otf.writeApo2(params.zoomfact * imgParams.nx, params.zoomfact * imgParams.ny,params.z_zoom * imgParams.nz0,sqrt(kx^2+ky^2)) ;

% 将每级OTF乘以截止区域,然后求共轭，再乘以对应的平移之后的频谱Dm，作为weiner的分子项
for order = -(params.norders - 1) : (params.norders - 1)
    kx = order * k0(dir).x;
    ky = order * k0(dir).y;
...
%     OTF = zeros(size(shifted(:, :, :, 2 * abs(order) + 1)), 'single');
    ...
    if order~=0
        % 为了将高级频谱的基频滤掉(否则会有条纹)，用以条纹矢量为圆心的幂函数来滤波
        apo3 = otf.writeApo3(params.zoomfact * imgParams.nx, params.zoomfact * imgParams.ny,params.z_zoom * imgParams.nz0,-kx,-ky) ;
        OTF = OTF.*apo3;
        if dir==1
            OTF = OTF.*apo2;            % 这一行决定要不要对高级频谱进行截止
%             OTF = 0;
        end
    end
    if (order == 0)
...
%     if (order < 0)
%         shifted(:, :, :, 2 * abs(order)) = shifted(:, :, :, 2 * abs(order)) + shifted(:, :, end:-1:1, 2 * abs(order));
%     elseif (order > 0)
%         shifted(:, :, :, 2 * abs(order) + 1) = shifted(:, :, :, 2 * abs(order) + 1) + shifted(:, :, end:-1:1, 2 * abs(order) + 1);
%     end
...
% saveWienerResult_single_direction_shifted(dir, it, gather(result), params, imgParams);
...
for dir = 1 : params.ndirs
    ...
    for order = -(params.norders - 1) : (params.norders - 1)
        kx = gather(order * k0(dir).x);
        ky = gather(order * k0(dir).y);
...
%         OTF = zeros(size(outbuffer), 'single');
        ...
        z0 = -imgParams.zdistcutoff(abs(order) + 1) : imgParams.zdistcutoff(abs(order) + 1);
        iz = mod((z0 + params.z_zoom * imgParams.nz0), params.z_zoom * imgParams.nz0);
        OTF(:, :, iz + 1) = gather(otfval);
        denom = denom + OTF;
    end
end

outbuffer = gather(fullResult) ./ denom;

if (params.apodizeoutput)
    apo = zeros(size(outbuffer), 'single');
    apofact = otf.writeApoFactor(params.zoomfact * imgParams.nx, params.zoomfact * imgParams.ny, 0, 0, imgParams.zapocutoff, gather(imgParams.apocutoff), imgParams.zapocutoff, params);
...
% 如果只有一个方向，为保证各向同性，用零级的圆柱形mask滤掉外围频谱
% if params.ndirs == 1
%    apo2 = otf.writeApo2(params.zoomfact * imgParams.nx, params.zoomfact * imgParams.ny,params.z_zoom * imgParams.nz0,imgParams.rdistcutoff) ;
%    outbuffer = outbuffer .* apo2;
% end
