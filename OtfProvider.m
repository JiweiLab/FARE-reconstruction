...
%             if()
            ...
            t = single(find(obj.vals(:),1)*0);          % 用来判断是否是GPU运行的
            if obj.bRadAvgOTF
                x = t : (nx - 1);
                y = t : (ny - 1);
                z = -nz : nz;
        ...
        %% 为只有一个dir的数据创建正弦的圆柱形mask，只需要把圆形的rep成nz层就行
        function apofact = writeApo2(obj, nx, ny, nz, apocutoff)
            x = 0 : (nx - 1);
            y = 0 : (ny - 1);
            x(x >= (nx / 2)) = x(x >= (nx / 2)) - nx;
            y(y >= (ny / 2)) = y(y >= (ny / 2)) - ny;
            ...
            rdist = sqrt(X .* X + Y .* Y);
            rho = zeros(nx,ny);
            rho(rdist<apocutoff) = 1;
            apofact = repmat(rho,[1 1 nz]);
             

        end
        %% 为了将高级频谱的基频滤掉，用以条纹矢量为圆心的圆环形的正弦函数来滤波
        function apofact = writeApo3(obj, nx, ny, nz, kx,ky)
            x = 0 : (nx - 1);
            y = 0 : (ny - 1);
            x(x >= (nx / 2)) = x(x >= (nx / 2))-nx;
            y(y >= (ny / 2)) = y(y >= (ny / 2))-ny;
            [X, Y] = meshgrid(x, y);
            rdist = sqrt((X-kx) .* (X-kx) + (Y-ky) .* (Y-ky));
            k = sqrt(kx*kx+ky*ky);
%             rdist(rdist>2*k) = 0;
%             rho = rdist .* rdist .* sin(pi*rdist/k);      % 用r^2sinr滤
            rho = rdist.^4/(k/1.5)^4;rho(rho>1) = 1;
            rho = rho/max(rho(:));
            apofact = repmat(rho,[1 1 nz]);
